...
program
  ...
  .option('-o, --output <path>', 'Specify the output directory for tests', 'test')
  .description('Analyze the project structure and generate tests')
  .action(async (projectPath, options) => {
    try {
      const generateUnit = options.unit || (!options.unit && !options.integration);
      const generateIntegration = options.integration || (!options.unit && !options.integration);
      ...
      if (generateIntegration) {
        ...
        await integrationTestGenerator.generate(`${options.output}/integration`);
